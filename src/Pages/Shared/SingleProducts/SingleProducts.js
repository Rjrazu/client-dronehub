...
const SingleProducts = ({product}) => {
    ...
    const {price , model , image , description , _id} = product
    return (
        <Grid item xs={12} sm={6} md={4} className="text-center">
            <Box className="p-2 border h-100">
            <img src={image} alt="" style={{width:"250px", height: "150px"}}/>
            <Box className="p-3">
            <h3 className="my-3" style={{color:grey[800]}}>{model}</h3>
            <p style={{color:grey[600]}} className="fs-4 fw-bold">$ {price}</p>
            <p style={{color:grey[800] , fontSize:'17px'}}> {`${description.slice(0,100)}...`}</p>
